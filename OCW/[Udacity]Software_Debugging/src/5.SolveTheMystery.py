...
answer_value = 0.8165   # precision to 4 decimal places. (0.816496580927726)
...
def traceit_mystery(frame, event, arg):
    global func_coverage

    if event == "return":
        func_name = frame.f_code.co_name
        ret = arg
        if not func_coverage.has_key(func_name):
            func_coverage[func_name] = {-1:0, 0:0, 1:0}

        if type(ret) == int:
            if ret > 0:
                func_coverage[func_name][1] = func_coverage[func_name][1] + 1
            elif ret == 0:
                func_coverage[func_name][0] = func_coverage[func_name][0] + 1
            elif ret < 0:
                func_coverage[func_name][-1] = func_coverage[func_name][-1] + 1
        elif type(ret) == str:
            if len(ret) > 0:
...
def run_tests_mystery(inputs):
    ...
    for ith_input in inputs:
        ...
        ret = mystery(ith_input)
        sys.settrace(None)
        for ith_func in func_coverage:
            if ith_func != 'mystery':
                runs.append((ith_func, ret, func_coverage[ith_func]))

    return runs

# tables['f1'/'f2'/'f3'][-1/0/1]['PASS'/'FAIL']['Covered'/'Uncovered'] = 0
def init_tables_mystery(runs):
    tables = {}
    for (ith_func, ith_res, ith_times) in runs:
        if ith_func not in tables:
            tables[ith_func] = {-1:set(), 0:set(), 1:set()}
            for jth_val in tables[ith_func]:
                tables[ith_func][jth_val] = {'PASS':set(), 'FAIL':set()}
                for kth_res in tables[ith_func][jth_val]:
                    tables[ith_func][jth_val][kth_res] = {'Covered':0, 'Uncovered':0}

        ith_summation = ith_times[-1] + ith_times[0] + ith_times[1]
        for jth_val in tables[ith_func]:
            tables[ith_func][jth_val][ith_res]['Covered'] = tables[ith_func][jth_val][ith_res]['Covered'] + ith_times[jth_val]
            tables[ith_func][jth_val][ith_res]['Uncovered'] = tables[ith_func][jth_val][ith_res]['Uncovered'] + (ith_summation - ith_times[jth_val])

    return tables

# phi_tables['f1'/'f2'/'f3'][-1/0/1]
def compute_n_mystery(tables):
    phi_tables = {}
    for ith_func in tables:
        if ith_func not in phi_tables:
            phi_tables[ith_func] = {-1:0, 0:0, 1:0}

        for jth_val in tables[ith_func]:
            n11 = tables[ith_func][jth_val]['FAIL']['Covered']
            n10 = tables[ith_func][jth_val]['PASS']['Covered']
            n01 = tables[ith_func][jth_val]['FAIL']['Uncovered']
            n00 = tables[ith_func][jth_val]['PASS']['Uncovered']
            try:
                phi_tables[ith_func][jth_val] = phi(n11, n10, n01, n00)
            except:
                phi_tables[ith_func][jth_val] = None

    return phi_tables

runs = run_tests_mystery(inputs)
tables = init_tables_mystery(runs)
phi_tables = compute_n_mystery(tables)
print phi_tables
