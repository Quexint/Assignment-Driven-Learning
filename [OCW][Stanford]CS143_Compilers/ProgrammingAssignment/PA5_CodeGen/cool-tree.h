...
typedef SymbolTable<Symbol, Location> Environment;
#define reg(x) (x).first
#define offset(x) (x).second
typedef std::vector<attr_class*> AttrVector;
typedef std::pair<CgenNode*, method_class*> ClassMethodPair;
typedef std::vector<ClassMethodPair> MethodVector;
...
typedef std::map<Symbol, Symbol> SymbolMappingSymbol;
typedef SymbolTable<Symbol, Symbol> TypeEnv;
class VariableEnvironment;
class CgenClassTable;

#define CgenParameter CgenNode* obj_self, VariableEnvironment *cgen_env, CgenClassTable *cctable, ostream &s
#define CgenParameterPrototype CgenNode*, VariableEnvironment*, CgenClassTable *, ostream&
#define CgenParameterInstance obj_self, cgen_env, cctable, s
...
public:
  ...
  void push_reg_into_stack(Symbol name, char *addr, CgenParameter);
  void pop_stack(char *addr, CgenParameter);
  void alloc_stack(bool output, int x, CgenParameter);
  void free_stack(bool output, int x, CgenParameter);
  int get_stack_base(){ return stack_base; }

  void import_class_formal(CgenParameter);

  Location lookup_env(Symbol name);
  Symbol lookup_typ(Symbol name);
  void addid_env(Symbol key, Location *val);
  void addid_typ(Symbol key, Symbol *val);
  void enterscope();
  void exitscope();
  void dump_env();
